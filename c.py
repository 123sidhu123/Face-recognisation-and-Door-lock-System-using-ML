...
# -- Added buffer cleanup (shutil.rmtree) after email process
# -- Added tuning notes for blink detection
...
# If blink detection is not working well, this is the value to change.
# Try lowering it (e.g., 0.28, 0.25) if blinks are not detected.
# Try raising it (e.g., 0.32) if it's too sensitive.
EYE_AR_THRESH_OPEN = 0.30
# !!! ================================ !!!

EYE_AR_THRESH_CLOSE = 0.20 # Note: This variable is not used in the current logic
    ...
    try:
        while True:
            ...
            if blink_detected:
                ...
                # ========== BUFFER CLEANUP (CHANGE 1) ==========
                # Get the path to the buffer folder that was just created
                buffer_folder_path = save_unknown_faces_buffer(frames_list, unique_id)
                # ===============================================
                ...
                if all_same:
                    ...
                    # ========== BUFFER CLEANUP (CHANGE 2) ==========
                    # Always delete the buffer folder after the email process is done
                    if buffer_folder_path and os.path.exists(buffer_folder_path):
                        print(f"[CLEANUP] Removing buffer directory: {buffer_folder_path}")
                        try:
                            shutil.rmtree(buffer_folder_path)
                        except Exception as e:
                            print(f"[CLEANUP] Error removing buffer: {e}")
                    # ===============================================
                            
                else:
                    print("[FAILED] Frames don't match\n")
                    # Also clean up if frames didn't match
                    if buffer_folder_path and os.path.exists(buffer_folder_path):
                        print(f"[CLEANUP] Removing buffer (frames did not match): {buffer_folder_path}")
                        try:
                            shutil.rmtree(buffer_folder_path)
                        except Exception as e:
                            print(f"[CLEANUP] Error removing buffer: {e}")
