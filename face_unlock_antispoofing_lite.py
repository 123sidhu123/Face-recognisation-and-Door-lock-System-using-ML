# face_unlock_antispoofing_lite.py
# LIGHTWEIGHT VERSION - Works without dlib model
# Uses simplified liveness detection (texture + motion only)
# Download dlib model later for full anti-spoofing
...
print("""
╔════════════════════════════════════════════════════════════════════╗
║   FACE RECOGNITION DOOR LOCK - LIGHTWEIGHT VERSION                ║
║   ✅ Works immediately (no dlib model needed)                     ║
║   ✅ Basic anti-spoofing: Texture + Motion Detection              ║
║   ✅ Upgrade to full version after model download                 ║
╚════════════════════════════════════════════════════════════════════╝
""")
...
def check_texture_quality(frame, face_location):
    """Check texture variance to detect flat images"""
...
def detect_screen_playback(prev_frame, curr_frame, face_location):
    """Detect video playback by checking frame differences"""
    if prev_frame is None:
        return False

    top, right, bottom, left = face_location

    if top >= prev_frame.shape[0] or bottom > prev_frame.shape[0]:
        return False
    if left >= prev_frame.shape[1] or right > prev_frame.shape[1]:
        return False
    ...
    return avg_diff < FRAME_DIFF_THRESHOLD


def check_color_saturation(frame, face_location):
    """Check color saturation to detect printed photos"""
    top, right, bottom, left = face_location
    face_region = frame[top:bottom, left:right]
...
✅ LIVENESS CONFIRMED (Real face - not photo/video)
...
def wait_for_email_reply(request_id):
...
def main():
    ...
    print("="*70)
    print("LIGHTWEIGHT VERSION (Basic Anti-Spoofing) - Press 'q' to quit")
    print("="*70)
    print("⚠️  For full blink detection, run: python setup_dlib_model.py\n")
    ...
    try:
        while True:
            ...
            for idx, face_enc in enumerate(face_encodings):
                ...
                cv2.putText(frame, f"Texture: {texture_var:.1f}", (10, 50), 
                ...
                if texture_var < TEXTURE_VARIANCE_THRESH:
                    is_spoof = True
                    cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 3)
                    cv2.putText(frame, "❌ FAKE (Low Texture)", (left, top - 10),
                ...
                if check_color_saturation(frame, (top, right, bottom, left)):
                    is_spoof = True
                    cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 3)
                    cv2.putText(frame, "❌ FAKE (Low Saturation)", (left, top - 10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    motion_detected = False
                    consecutive_unknown_counter = 0
                    continue
                ...
                if prev_gray_frame is not None:
                    if detect_screen_playback(prev_gray_frame, gray, (top, right, bottom, left)):
                        is_spoof = True
                        cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 3)
                        cv2.putText(frame, "❌ FAKE (Video Playback)", (left, top - 10),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        motion_detected = False
                        consecutive_unknown_counter = 0
                        continue
                ...
                motion_history.append(np.array([center_x, center_y]))

                if len(motion_history) >= 10:
                    motion_array = np.array(motion_history)
                    motion_var = np.var(motion_array)
                    motion_detected = motion_var > MIN_MOTION_VARIANCE
                else:
                    motion_var = 0

                cv2.putText(frame, f"Motion: {motion_var:.1f}", (10, 80), 
                ...
                if detected and best_match:
                    ...
                    if motion_detected:
                        ...
                        cv2.putText(frame, f"✓ {best_match} (LIVE)", (left, top - 10),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                    else:
                        cv2.rectangle(frame, (left, top), (right, bottom), (255, 165, 0), 3)
                        cv2.putText(frame, f"⏳ {best_match} (motion?)", (left, top - 10),
                ...
                # === UNKNOWN FACE ===
                    ...
                    if motion_detected:
                        ...
                        cv2.putText(frame, "❓ Unknown (LIVE)", (left, top - 10),
                    ...
                    if encoding_distance < 0.4 and motion_detected:
                ...
                # === SAVE FRAME ONLY IF MOTION DETECTED ===
                if consecutive_unknown_counter > 0 and consecutive_unknown_counter <= UNKNOWN_FACES_THRESHOLD:
                    if motion_detected and (current_time - last_frame_time) >= FRAME_CAPTURE_GAP:
                        save_full_frame(frame)
                        last_frame_time = current_time
                        print(f"[BUFFER] ✓ Frame saved ({consecutive_unknown_counter}/{UNKNOWN_FACES_THRESHOLD})")
            ...
            if buffer_count >= UNKNOWN_FACES_THRESHOLD and not verification_done:
                ...
                if all_same:
                    ...
                    if send_approval_email(buffer_photos, similarity, unique_id):
                        reply = wait_for_email_reply(unique_id)
            ...
            if matched_this_frame and matched_this_frame == last_matched_person:
                ...
                consecutive_counter = 1 if motion_detected else 0
            else:
                last_matched_person = None
                consecutive_counter = 0

            # === UNLOCK ===
            if consecutive_counter >= CONSECUTIVE_CONFIRMATION and motion_detected:
                print(f"\n✅ {last_matched_person} matched AND LIVE!")
            ...
            cv2.putText(frame, f"Match: {consecutive_counter}/{CONSECUTIVE_CONFIRMATION}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)

            buffer_count = count_buffer_frames()
            if buffer_count > 0:
                cv2.putText(frame, f"Buffer: {buffer_count}/5", 
                           (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 100, 200), 1)

            cv2.imshow("🔓 Lightweight Anti-Spoofing Lock", frame)
